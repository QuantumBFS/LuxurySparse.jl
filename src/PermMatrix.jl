...
`PermMatrix` represents a special kind linear operator: Permute and Multiply,
which means `M * v = v[perm] * val`.
...
struct PermMatrix{Tv, Ti<:Integer, Vv<:AbstractVector{Tv}, Vi<:AbstractVector{Ti}} <: AbstractLuxurySparseMatrix{Tv, Ti}
    ...
    function PermMatrix{Tv, Ti, Vv, Vi}(perm::Vi, vals::Vv) where {Tv, Ti, Vv, Vi}
